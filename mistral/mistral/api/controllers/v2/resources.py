...
class Report(resource.Resource):
    ...
    workflow_state = wtypes.text
    ("state can take one of the following values: "
     "IDLE, RUNNING, SUCCESS, ERROR, DELAYED")

    workflow_state_info = types.jsontype
    "state information string"

    workflow_traceback = wtypes.text
    "an optional verbose state information"
    ...
    task_state_info = types.jsontype
    "an optional state information string"

    @classmethod
    def sample(cls):
        return cls(workflow_name='flow',
                   workflow_id='123e4567-e89b-12d3-a456-426655441111',
                   workflow_state=states.ERROR,
                   workflow_state_info={'task_name': 'error cause'},
                   task_name='task1',
                   task_execution_id='123e4567-e89b-12d3-a456-426655440000',
                   task_state=states.ERROR,
                   task_state_info={'action': 'error cause'},
                   workflow_traceback='This is where traceback info will be,'
                   'reported.')
